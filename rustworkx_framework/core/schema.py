...
class SchemaVersion(semver.Version):
    """
    Версия схемы на базе semver.

    Делегирует парсинг, форматирование и все операторы сравнения
    в ``semver.Version`` — никакой ручной логики разбора строк.
    ...
    def is_schema_compatible(self, other: "SchemaVersion") -> bool:
        """Совместимость по мажорной версии (semver major)."""
        return self.major == other.major

    @classmethod
    def current(cls) -> "SchemaVersion":
        """Вернуть текущую версию схемы фреймворка."""
...
class BaseNodeSchema(BaseModel):
    ...
    def auto_migrate(cls, data: Any) -> Any:
        """Автоматически применить миграции при десериализации устаревших данных."""
