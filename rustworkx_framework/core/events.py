...
class EventHandler[T: Event](ABC):
    ...
    Attributes:
        raise_error: Если True — исключения из handle() всплывают наружу.
                     Если False (по умолчанию) — ошибки логируются и шина
                     продолжает работу со следующими обработчиками.
