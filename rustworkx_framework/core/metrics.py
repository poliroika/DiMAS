...
class EdgeMetrics(BaseModel):
    ...
    def get_effective_weight(
        ...
        return (
            reliability_factor * reliability_cost
            + latency_factor * latency_cost
            + base_factor * base_cost
        )
...
class MetricsTracker:
    ...
    def set_edge_custom_metric(
        self, source_id: str, target_id: str, name: str, value: float
    ) -> None:
    ...
    def get_unreliable_nodes(self, threshold: float = 0.5) -> list[str]:
        ...
        return [
            node_id for node_id, node in self._node_metrics.items() if node.reliability < threshold
        ]
