...
class GNNRouterInference:
    ...
    def predict(
        ...
        # node_features и edge_index уже являются Tensor, просто перемещаем на device
        ...
        # Handle multi-class output: use softmax over classes, take class-1 probability
        if out.dim() == 2 and out.shape[1] > 1:
            probs = functional.softmax(out, dim=1)
            scores = probs[:, -1].cpu()  # probability of "high-quality" class
        else:
