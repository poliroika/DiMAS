...
class RoleGraph(BaseModel):
    ...
    def role_sequence(self) -> list[str]:
        ...
        return [a.identifier for a in self.agents]

    @property
    def embeddings(self) -> torch.Tensor:
        """Стек эмбеддингов агентов или пустой тензор."""
        embs = [a.embedding for a in self.agents if a.embedding is not None]
    ...
    def _archive_state(self, agent: Any) -> None:
        ...
        state_data = {
            ...
            "embedding": (
                agent.embedding.cpu().tolist()
                if hasattr(agent, "embedding") and agent.embedding is not None
                else None
            ),
        }
        self.state_storage.save(agent.identifier, state_data)
            ...
            for j in range(n_nodes):
                if a_tensor[i, j].item() > 0.5 and i < len(node_indices) and j < len(node_indices):
                    ...
                    if s_tilde is not None:
                        s_tensor = (
                            s_tilde.detach().cpu() if s_tilde.requires_grad else s_tilde.cpu()
                        )
                        edge_data["score"] = float(s_tensor[i, j].item())
                    if p_matrix is not None:
                        p_tensor = (
                            p_matrix.detach().cpu() if p_matrix.requires_grad else p_matrix.cpu()
                        )
    ...
    def from_dict(
        ...
        for a_data in data.get("agents", []):
            ...
            agent = factory(
                identifier=a_data["identifier"],
                display_name=a_data.get("display_name", a_data["identifier"]),
        ...
        idx_map = {}
        for agent in agents:
            idx_map[agent.identifier] = graph.add_node(
                {
                    "id": agent.identifier,
                    "type": "agent",
                }
        ...
        rg = cls(
            agents=agents,
            node_ids=data.get("node_ids", [a.identifier for a in agents]),
    ...
    def from_graph(
        ...
        a_tensor = (
            a_com if isinstance(a_com, torch.Tensor) else torch.tensor(a_com, dtype=torch.float32)
        )
        rg = cls(
            agents=agents_list,
            node_ids=[a.identifier for a in agents_list],
    ...
    def to_pyg_data(
        ...
        if x_parts:
            ...
            x = torch.zeros((n, 0), dtype=torch.float32)
        ...
        if edge_features:
            for name, feat in edge_features.items():
        ...
        if ea_parts:
            ea = torch.cat(ea_parts, dim=1)
        else:
            ea = torch.zeros((ei.shape[1], 0), dtype=torch.float32)
    ...
    def get_edge_features_from_schema(self) -> dict[str, torch.Tensor]:
        ...
        return {
            name: torch.tensor(values, dtype=torch.float32) for name, values in features.items()
        }
    ...
    def subgraph(self, node_ids: list[str]) -> "RoleGraph":
        ...
        agents = [a for a in self.agents if a.identifier in node_ids]
        id_set = set(node_ids)
        new_graph = rx.PyDiGraph()
        idx_map = {}
        for agent in agents:
            old_idx = self.get_node_index(agent.identifier)
        ...
        new_connections = {
            k: [v for v in vs if v in id_set]
            for k, vs in self.role_connections.items()
            if k in id_set
        }
        return RoleGraph(
            agents=agents,
            node_ids=[a.identifier for a in agents],
    ...
    def get_optimized_execution_order(
        ...
        in_degree: dict[str, int] = {node_id: 0 for node_id in relevant}
