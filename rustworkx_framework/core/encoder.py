    ...
    def validate_model_name(cls, v: str) -> str:
        ...
        if v == _HASH_PROVIDER or v.startswith(_HASH_PREFIX):
            if v.startswith(_HASH_PREFIX):
                ...
                if not raw_dim.isdigit():
                    raise ValueError(f"Hash embedding dimension must be numeric, got {raw_dim!r}")
                if int(raw_dim) < 1:
                    raise ValueError(f"Hash embedding dimension must be positive, got {raw_dim}")
            return v
        ...
        if any(v.startswith(prefix) for prefix in _SENTENCE_TRANSFORMERS_PREFIXES):
            ...
            if len(parts) == 2 and parts[1].strip():
                return v
            raise ValueError(
                f"SentenceTransformer specification '{v}' is missing the model identifier"
            )

        raise ValueError(
            "Unsupported embedding model. Expected 'sentence-transformers/<model>' or 'hash[:<dim>]'"
        )
