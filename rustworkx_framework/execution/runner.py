...
# ---------------------------------------------------------------------------
#: Минимальный вес ребра, ниже которого оно считается отсутствующим (BFS).
...
class LLMCallerFactory:
    ...
    def _merge_config(self, config: AgentLLMConfig) -> AgentLLMConfig:
        """Слить конфигурацию агента с конфигурацией по умолчанию."""
    ...
    def get_caller(
        ...
        Args:
            ...
            _agent_id: ID агента (зарезервировано для будущего использования).
...
class ExecutionContext(NamedTuple):
    """
    Общий контекст инициализации для всех методов выполнения.

    Формируется методом ``MACPRunner._prepare_base_context`` и содержит
    разобранные данные графа: индексы, идентификаторы агентов и lookups.
    """
...
class MACPRunner:
    ...
    def _prepare_base_context(self, role_graph: Any) -> "ExecutionContext | None":
        """
        Собрать общий контекст инициализации из role_graph.

        Извлекает task_idx, матрицу смежности, список agent_ids, запрос
        и lookup-словари. Не вызывает ``_init_memory`` — каждый execution-метод
        делает это сам (с учётом возможной фильтрации агентов).

        Returns:
            ``ExecutionContext`` с данными графа, или ``None`` если агентов нет.
    ...
    def _run_adaptive(  # noqa: PLR0912, PLR0915
        ...
        *,
        ...
        and context-manager callbacks (аналогично ``_run_simple``).
        """
        if not self._has_any_caller():
            msg = "llm_caller, llm_callers, or llm_factory is required for synchronous execution"
            raise ValueError(msg)
        ...
        # Инициализация callbacks (per-call + config + context-manager)
        run_id = self._init_run(
            graph_name=getattr(role_graph, "name", None),
            ...
            while not plan.is_complete:
                ...
                if self._callback_manager:
                    ...
                    if _agent_obj is not None:
                        ...
                        _prompt_preview = self._build_prompt(_agent_obj, query, _inc, agent_names, _mem)
                    self._callback_manager.on_agent_start(
                        run_id=run_id,
                        agent_id=step.agent_id,
                        agent_name=agent_name,
                        step_index=step_idx,
                        prompt=_prompt_preview[: self.config.prompt_preview_length],
    ...
    async def _arun_adaptive(  # noqa: PLR0912, PLR0915
        ...
        *,
        ...
        Supports filtering of isolated nodes to save tokens.
        Per-call ``callbacks`` are merged with ``RunnerConfig.callbacks``
        and context-manager callbacks (аналогично ``_arun_simple``).
        """
        if not self._has_any_async_caller():
            msg = "async_llm_caller, async_llm_callers, or llm_factory is required for async execution"
            raise ValueError(msg)
        ...
        try:
            while not plan.is_complete:
                ...
                if not valid_steps:
                    # Все шаги группы пропущены или pruned — план может зависнуть.
                    # Если plan.is_complete уже True — выйдем на следующей итерации;
                    # иначе явно прерываем, чтобы не зациклиться.
                    break
    ...
    async def _astream_simple(  # noqa: PLR0915
        self,
        role_graph: Any,
        final_agent_id: str | None,
        *,
    ...
    async def _astream_adaptive(  # noqa: PLR0912, PLR0915
        ...
        *,
        ...
        while not plan.is_complete:
            ...
            if not valid_steps:
                # Все шаги группы пропущены или pruned — прерываем, чтобы не зависнуть.
                break
