...
class ConditionEvaluator:
    ...
    def _register_builtins(self) -> None:
        ...
        # Всегда True/False
        self._named_conditions["always"] = lambda ctx: True
        self._named_conditions["never"] = lambda ctx: False
...
class ExecutionPlan(BaseModel):
    ...
    def remaining_steps(self) -> list[ExecutionStep]:
        ...
        return [
            ...
            if step.agent_id not in self.completed and step.agent_id not in self.skipped
        ]
    ...
    def _advance(self) -> None:
        """Сдвинуть current_index до следующего невыполненного шага."""
        while self.current_index < len(self.steps) and self.steps[self.current_index].agent_id in (
    ...
    def insert_fallback(self, fallback_agent_id: str, after_index: int) -> None:
        ...
        if fallback_agent_id in self.completed or fallback_agent_id in self.skipped:
            return
...
def filter_reachable_agents(
        ...
        while queue:
            ...
            for j, aid in enumerate(agent_ids):
                if aid not in reachable_from_start:
                    if a_agents[current_idx, j].item() > threshold:
    ...
    if effective_end in agent_ids:
        ...
        while queue:
            ...
            current_idx = agent_ids.index(current)
            for i, aid in enumerate(agent_ids):
                if aid not in reaching_end:
                    if a_agents[i, current_idx].item() > threshold:
...
def build_execution_order(
    ...
    if num_agents != len(agent_ids):
        raise ValueError(f"a_agents size {num_agents} != agent_ids length {len(agent_ids)}")
    ...
    for scc_idx in scc_order:
        ...
        scc_agents.sort(key=lambda a: fallback_rank.get(a, len(fallback)))
...
def get_parallel_groups(
    ...
    while not torch.all(executed):
        ready: list[str] = []
        for i in range(num_agents):
            if not executed[i] and remaining_in[i] == 0:
...
class AdaptiveScheduler:
    """Планировщик с разными политиками маршрутизации и поддержкой replanning.
    ...
    def build_plan(
        ...
        # Применить условия к матрице, если заданы
        effective_a = a_agents
        if edge_conditions:
            effective_a = self._apply_conditions(
                a_agents.clone(), agent_ids, edge_conditions, condition_context
            )
        ...
        if self.policy == RoutingPolicy.GREEDY:
            ...
            order = self._weighted_topological_order(effective_a, effective_agent_ids, effective_p)
        else:
            order = build_execution_order(effective_a, effective_agent_ids)

        steps: list[ExecutionStep] = []
        for agent_id in order:
            idx = effective_agent_ids.index(agent_id)
            predecessors = get_incoming_agents(
            ...
            fallbacks = self._find_fallback_agents(
                agent_id, effective_agent_ids, effective_a, order
            )
    ...
    def should_prune(
        ...
        if self.pruning.token_budget is not None:
            if plan.tokens_used >= self.pruning.token_budget:
                return (
                    True,
        ...
        if self.pruning.skip_on_predecessor_failure:
            ...
            if failed_predecessors and not step.is_optional:
                if not step.fallback_agents or not self.pruning.enable_fallback:
    ...
    def should_use_fallback(
        ...
        if not result.success:
            return True
        if result.quality_score < self.pruning.min_quality_threshold:
            return True
        return False
    ...
    def replan(
        ...
        """Перепланировать оставшиеся шаги с учётом результатов и корректировок весов."""
        remaining_agents = [
            step.agent_id
            for step in plan.remaining_steps
            if step.agent_id not in plan.completed and step.agent_id not in plan.skipped
        ]

        if not remaining_agents:
            return plan

        adjusted_a = self._adjust_weights_from_results(
            a_agents.clone(), agent_ids, step_results or {}
        )

        adjusted_p = None
        if p_matrix is not None:
            adjusted_p = self._adjust_probabilities_from_results(
                p_matrix.clone(), agent_ids, step_results or {}
            )

        remaining_indices = torch.tensor([agent_ids.index(aid) for aid in remaining_agents])
        sub_a = adjusted_a[remaining_indices][:, remaining_indices]
        sub_p = (
            adjusted_p[remaining_indices][:, remaining_indices] if adjusted_p is not None else None
        )

        new_plan = self.build_plan(sub_a, remaining_agents, sub_p)

        for step in new_plan.steps:
            step.predecessors = [p for p in step.predecessors if p in remaining_agents]

        new_plan.completed = plan.completed.copy()
        new_plan.failed = plan.failed.copy()
        new_plan.skipped = plan.skipped.copy()
        new_plan.tokens_used = plan.tokens_used
    ...
    def _weighted_topological_order(
        ...
        in_weights = torch.sum(combined, dim=0)
        out_weights = torch.sum(combined, dim=1)
        priorities = out_weights - in_weights
        ...
        try:
            ...
            indices = torch.argsort(-priorities)
            return [agent_ids[i] for i in indices.tolist()]
    ...
    def _k_shortest_order(
        ...
        end_idx = (
            agent_ids.index(end_agent) if end_agent and end_agent in agent_ids else num_agents - 1
        )
        ...
        try:
            ...
            if node_indices[end_idx] in paths:
                ...
                remaining = [aid for aid in agent_ids if aid not in order]
