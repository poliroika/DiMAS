...
# ---------------------------------------------------------------------------
#: Малая добавка для предотвращения деления на ноль при вычислении весов рёбер.
_EPSILON: float = 1e-6
#: Порог веса по умолчанию для определения наличия ребра.
_DEFAULT_WEIGHT_THRESHOLD: float = 0.5
#: Порог схожести по умолчанию для семантических сравнений.
...
def filter_reachable_agents(  # noqa: PLR0912
    ...
    # Предварительный маппинг для O(1) поиска индексов
...
class AdaptiveScheduler:
    ...
    def should_prune(
        ...
        # Динамический прунинг по quality_score предыдущего шага:
        # если качество ответа предшественника ниже порога — пропускаем текущий шаг.
        if (
