...
class ReplanEvent(StreamEvent):
    """Emitted when execution plan is updated."""

    event_type: Literal["replan"] = "replan"
    reason: str = ""
...
def format_event(event: StreamEvent, verbose: bool = False) -> str:
    ...
    if isinstance(event, RunStartEvent):
        ...
        return f"[{timestamp}] 🔄 Replan #{event.replan_count}: {event.reason}"
    ...
    elif isinstance(event, ParallelStartEvent):
...
def print_stream(
    ...
    for event in stream:
        ...
        if isinstance(event, TokenEvent):
            if show_tokens:
                if current_agent != event.agent_id:
                    if current_agent is not None:
                        print()  # New line for new agent
                    current_agent = event.agent_id
                print(event.token, end="", flush=True)
                if event.is_last:
                    print()  # End line after last token
            ...
            if current_agent is not None:
                print()  # End any in-progress token stream
                current_agent = None
            print(format_event(event, verbose))
