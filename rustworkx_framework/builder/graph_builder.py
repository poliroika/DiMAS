    ...
    def add_agent(
        ...
        **metadata,
        ...
        Args:
            ...
            # LLM Configuration (для мультимодельности):
        ...
        """
        # Если передан llm_config, используем его значения как fallback
...
def build_from_schema(
    ...
    connections = {a.identifier: [] for a in agents}
    edge_condition_names: dict[tuple[str, str], str] = {}

    for edge_schema in schema.edges:
        if edge_schema.source in connections:
            if edge_schema.target not in connections[edge_schema.source]:
                connections[edge_schema.source].append(edge_schema.target)

        # Извлечь строковые условия из WorkflowEdgeSchema
        if hasattr(edge_schema, "condition") and edge_schema.condition:
            edge_condition_names[(edge_schema.source, edge_schema.target)] = edge_schema.condition  # type: ignore[assignment]
...
def build_property_graph(
    ...
    *,
    ...
    if anchor is None and agents:
        anchor = agents[0].identifier

    for i, agent in enumerate(agents):
        extra = (node_features or {}).get(agent.identifier, {})
        trust = 1.0 + (0.1 if agent.identifier == anchor else 0)
    ...
    pos = {a.identifier: i for i, a in enumerate(agents)}
...
def build_from_adjacency(
    ...
    *,
    ...
    if tuple(adjacency.shape) != (n, n):
        raise ValueError(f"Adjacency shape {tuple(adjacency.shape)} doesn't match {n} agents")

    edges = []
    for i in range(n):
        for j in range(n):
            if i != j and adjacency[i, j] > threshold:
                edges.append((agents[i].identifier, agents[j].identifier))
