...
class RaisingHandler(BaseCallbackHandler):
    ...
    def on_run_start(self, **_: Any) -> None:  # type: ignore[override]
...
def test_callback_manager_ignore_flags_filter_events() -> None:
    ...
    m = CallbackManager(
        handlers=[h_ok, h_ignore_retry, h_ignore_llm, h_ignore_memory, h_ignore_budget]
    )
...
def test_callback_manager_error_in_one_handler_does_not_block_others_when_raise_error_false() -> (
    None
):
